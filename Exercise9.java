 ...
 *	This program calculates the slope of a line passing through two user-supplied integer 
 *  cordinates (X1, Y1) and the end-point coordinates (0, 0).
 *
 */
import java.awt.*;
import javafx.util.*;
...
public class Exercise9 {
    ...
    private static int getCoordinate(String msg) {
	    ...
	    do{	    
	    	...
	    	kv = tryParseInt(numStr);
	    	
	    	if(!kv.getKey()){
	    		label = new JLabel(String.format("Not a valid number\n"));	    			    
    ...
    private static String slope(int x1, int y1) {
		...
		return String.format("%.3f", numerator / denumerator);
    }
    //----------------------------------------------------------------
